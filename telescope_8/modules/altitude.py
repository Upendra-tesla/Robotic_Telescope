...
# --------------------------
# Altitude Control Thread (Thread-Safe)
# --------------------------
class AltitudeThread(QThread):
    ...
    def __init__(self):
        ...
        self.motor = None
        self.pwm = None
        self.altitude_pins = SETTINGS["gpio"]["altitude_pins"]
    ...
    def init_motor(self, pins=None):
        """Initialize altitude motor (Pi 5 Compatible - No Pigpio)"""
        locker = QMutexLocker(self.mutex)
        
        # Update pins if provided
        if pins:
            self.altitude_pins = pins
            SETTINGS["gpio"]["altitude_pins"] = pins
            save_settings()
        ...
        try:
            if self.gpio_available:
                ...
                self.motor = self.GPIOMotor(
                    forward=self.altitude_pins["in1"],
                    backward=self.altitude_pins["in2"]
                )
                self.pwm = self.PWMOutputDevice(
                    pin=self.altitude_pins["ena"],
                    frequency=SETTINGS["gpio"]["pwm_frequency"]
                )
                self.pwm.value = self.speed / 100  # Convert % to 0-1 range
                
                logger.info(f"Altitude motor initialized (Pi 5) - Pins: IN1={self.altitude_pins['in1']}, IN2={self.altitude_pins['in2']}, ENA={self.altitude_pins['ena']}")
                self.status_update.emit(f"Motor Ready (Pins: {self.altitude_pins['in1']}/{self.altitude_pins['in2']}/{self.altitude_pins['ena']})")
            else:
    ...
    def set_altitude(self, target):
        """Set target altitude (0-90 degrees)"""
        locker = QMutexLocker(self.mutex)
        # Validate target range
        self.target_position = max(0, min(90, int(target)))
    ...
    def run(self):
        ...
        self.running = True
        logger.info("Altitude thread started")
            ...
            try:
                ...
                if self.motor and self.current_position != self.target_position:
                    ...
                    if self.current_position < self.target_position:
                        # Move UP
                        self.motor.forward()
                        self.current_position += 0.1  # Simulated position increment
                        self.status_update.emit(f"Moving UP - {self.current_position:.1f}°")
                    else:
                        # Move DOWN
                        self.motor.backward()
                        self.current_position -= 0.1  # Simulated position increment
                        self.status_update.emit(f"Moving DOWN - {self.current_position:.1f}°")
    ...
    def stop(self):
        ...
        self.running = False
        self.stop_motors()
        logger.info("Altitude thread stopped")

# --------------------------
# Altitude Control Widget (800x480 Touchscreen Optimized)
# --------------------------
class AltitudeControlWidget(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setStyleSheet(get_responsive_stylesheet())
        self.altitude_thread = None
        self.init_ui()
        self.init_thread()
    ...
    def init_ui(self):
        ...
        # Title
        title_label = QLabel("Altitude Control (0-90°)")
        ...
        self.in1_combo.setCurrentText(f"GPIO{SETTINGS['gpio']['altitude_pins']['in1']}")
        ...
        self.in2_combo.setCurrentText(f"GPIO{SETTINGS['gpio']['altitude_pins']['in2']}")
        ...
        self.ena_combo.setCurrentText(f"GPIO{SETTINGS['gpio']['altitude_pins']['ena']}")
        ...
        # Position Slider (0-90)
        self.position_slider = QSlider(Qt.Horizontal)
        self.position_slider.setRange(0, 90)
        ...
        self.position_bar.setRange(0, 90)
        ...
        self.down_btn.setMinimumHeight(40)
    ...
    def init_thread(self):
        ...
        self.altitude_thread = AltitudeThread()
        # Connect signals to UI
        self.altitude_thread.status_update.connect(self.update_status)
        self.altitude_thread.position_update.connect(self.update_position)
        self.altitude_thread.error_signal.connect(self.show_error)
    ...
    def apply_pins(self):
        ...
        try:
            ...
            self.altitude_thread.init_motor(SETTINGS["gpio"]["altitude_pins"])
    ...
    def on_slider_change(self, value):
        ...
        self.altitude_thread.set_altitude(value)
        self.update_status(f"Target Set: {value}°")

    def on_speed_change(self, value):
        """Handle speed slider change"""
        self.altitude_thread.set_speed(value)
        self.speed_label.setText(f"{value}%")

    def move_up(self):
        """Manual UP button"""
        current = self.position_slider.value()
        new_target = min(90, current + 5)
        self.position_slider.setValue(new_target)
        self.altitude_thread.set_altitude(new_target)

    def move_down(self):
        """Manual DOWN button"""
        current = self.position_slider.value()
        new_target = max(0, current - 5)
        self.position_slider.setValue(new_target)
        self.altitude_thread.set_altitude(new_target)

    def stop_motor(self):
        """Emergency stop button"""
        self.altitude_thread.stop_motors()
    ...
    def show_error(self, message):
        ...
        QMessageBox.critical(self, "Altitude Control Error", message)
        self.update_status(f"ERROR: {message}")

    def cleanup(self):
        """Cleanup thread and motor"""
        if self.altitude_thread:
            self.altitude_thread.stop()
            self.altitude_thread.wait()  # Wait for thread to finish
...
if __name__ == "__main__":
    ...
    window = AltitudeControlWidget()
    window.setWindowTitle("Altitude Control (Pi 5)")
